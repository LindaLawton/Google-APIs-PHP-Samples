...
// Option paramaters can be set as needed.
 $optParams = array(
  ...
  //'pageToken' => '[YourValue]',  // A token identifying a page of results that the server returns.Typically, this is the value of `ListAnalyticsResponse.next_page_token`returned from the previous call toListAnalytics.Will be a date string in `YYYY-MM-DD` format representing the end dateof the date range of results to return.If unspecified or set to "", default value is the current date.
            
  //'pageSize' => '[YourValue]',  // Requested page size. Server may return fewer analytics than requested.If unspecified or set to 0, default value is 30.Specifies the number of days in the date range when querying analytics.The `page_token` represents the end date of the date rangeand the start date is calculated using the `page_size` as the numberof days BEFORE the end date.Must be a non-negative integer.
...
// Single Request.
$results = analyticsListExample($service, $optParams);
...
*/
function analyticsListExample($service, $optParams)
{
	try
	{
		...
		return $service->analytics->ListAnalytics($optParams);
	}
	catch (Exception $e)
	{
